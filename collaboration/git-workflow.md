...
- Required machine-local validation uses `collaboration/local-execution-workflows.md`: Human Manual Mode through Qiven Operator while Host-mediated AI mutation is suspended; later the same Operator task/gate surface is invoked through the accepted Host authority path.
- Do not create unmanaged `%TEMP%` repository clones as routine validation materialization. Use the long-lived Qiven workspace plus Operator-owned exact-identity worktree/scratch lifecycle when isolation is needed.
...
A remembered SHA, local ref, cached state file, or JasonPC working copy never overrides live GitHub remote state.
...
Until this path is explicitly requalified, high-level Chat-side GitHub contents mutations are **not an accepted path for canonical merges or other critical repository writes**. Read-only GitHub connector operations remain allowed. For critical local mutation or human-assisted publication, use Human Manual Mode through Qiven Operator where the required capability exists. A low-level Git object/ref path may be used only when the exact repository, base/head identities, tree, parent set, and ref update are explicitly bound and reviewed; an ad-hoc contents write must never be used as a surrogate for a merge.
