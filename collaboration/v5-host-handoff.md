...
7. `4aca5b12031e0b9c2677738f67c929cb0bfa687d` — `host: quarantine competing broker session acquire`, parent exactly `be3a22e7...`; exact CI run `35057116028` passed Windows MSVC, Linux GCC, and macOS AppleClang in Debug and Release plus CI Gate. While already `Leased`, a different Host-owned `BrokerSessionId` is now treated as competing authority even if it presents the same client-controlled `ExecutionId`. The rejected competing Acquire transitions to durable `Quarantined`; a Windows restart regression proves the stronger phase survives broker replacement. Same-session/same-execution duplicate Acquire still returns `concurrent_execution` without leaving `Leased`.
...
The later competing-session Acquire correction makes the Host-established broker session authoritative over caller-selected execution identity for connection ownership. While `Leased`, a different stored/current `BrokerSessionId` is a competing authority even when the caller copies the same `ExecutionId`; missing stored session/execution identity in that phase also fails closed to `Quarantined`. A repeated Acquire from the same Host session and same execution is rejected as `concurrent_execution` but does not by itself imply split-brain. See `MEM-20260916T045200Z-C8F219`.
...
A rejected request is not assumed to leave authority unchanged. Exact review of `7e13bbbc...` caught a rejection path where wrong-session release had already changed `AuthorityState` to `Quarantined`; accepted correction `2faecefb...` compares pre/post phase and persists the stronger fail-closed state before returning the rejection. The same orchestration rule is what makes the later `4aca5b12...` competing-session Acquire correction durable: a rejected Acquire that strengthens the phase is persisted before its rejection evidence is returned. See `MEM-20260916T040700Z-D8A4C2`.
...
The competing-session correction adds a further boundary rule: a client may copy its own `ExecutionId`, so equality of caller-controlled identity cannot prove two requests originate from the same authority. The Host-owned connection session is the stronger evidence and must drive competitor classification where that ownership question is in scope.
...
Continue from exact Host checkpoint `4aca5b12031e0b9c2677738f67c929cb0bfa687d` with the owner-only bounded Windows named-pipe transport/session lifecycle as its own checkpoint before protected-operation dispatch.

The transport must keep its resource model bounded, reject remote clients, apply an owner-scoped local security boundary, and create a fresh nonzero Host-owned `BrokerSessionId` for each accepted connection. Session identity is not parsed from protocol frames. The transport design must preserve enough bounded concurrent connection capacity for a genuine second contender to reach later authority dispatch; do not hide split-brain by serializing all connection acceptance outside the authority state machine. A connected slot that loses transport or rejects framing must enter a fail-closed `Retiring` state and preserve the exact Host-owned session identity until upper-layer authority disconnect/reconciliation has been processed; only explicit retirement may clear that identity and reuse the slot.
