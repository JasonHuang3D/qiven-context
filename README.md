...
- `tools/`, `tests/` — validation machinery. The former retrieval/compiler tooling, kernel suites and benchmarks are sealed museum material under `legacy/` (ADR-0040/0041/0042).

All legacy surfaces (event ledger, deprecated evidence buckets, historical handoffs, sealed tooling, retired benchmarks) are consolidated under `legacy/` as museum material (ADR-0042); nothing there is an active write surface.
...
Passing one profile does not prove the others. K4 Canonical Artifact Handoff was accepted under `collaboration/context-handoff-contract.md`; the K5 lossless-transport goal is re-targeted to the Runtime canonical-cognition delivery path (ADR-0040, obligation OBL-20260920T235500Z-B6C1D8) and must preserve the K4 semantic closure exactly.

Registered views: `views/bindings/chatgpt-jason.yaml` (ChatGPT + Jason), `views/bindings/zcode-jason.yaml` (ZCode + Jason), and `views/bindings/extended-cognition-jason.yaml` (ZCode + Jason under the owner-granted extended-cognition designation; no model binding — owner-invoked). Different agents, humans, or machines use different views while reading the same canonical ProjectContext.
