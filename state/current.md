...
## Executable specification draft

ADR-0037 (2026-09-18) establishes `qiven-context-draft` (public, Devkit-adopted) as the executable C++ specification of the cognition architecture: `LLMCognition` as a pure value tree, admission-gated persistence with the typed handoff boundary, the minimal store contract with state-replication instead of event-sourcing, and participant changes as O(1) runtime rebinds. Its semantics are **candidate design input under validation** (`OBL-20260918T215000Z-B4D6A8`); until validation completes, all current context capabilities remain authoritative and there is no mechanism cutover. Operator coverage is complete (OBL-20260918T215000Z-C9E3F1): every Devkit-managed tool set runs through operator python tasks with `tools\qiven.cmd` as the only cmd entry, devkit self-hosts its own gate, and 0.1.4 rolls are gate-green in foundation, math and the draft. Bootstrap now requires asking the human operator when the applicable ContextView cannot be determined with confidence; assuming a participant combination is prohibited.
