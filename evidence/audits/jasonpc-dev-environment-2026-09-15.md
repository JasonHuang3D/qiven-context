...
- Development root observed: `D:\JasonWork`
...
| GitHub CLI | PATH exposes both `C:\Program Files\GitHub CLI\gh.exe` and `C:\Env\gh\bin\gh.exe`; invoked `gh` reported `2.100.0`; `gh auth status` succeeded for `JasonHuang3D`, Git protocol `ssh` |
| CMake | `C:\Env\cmake\bin\cmake.exe`; `4.4.3` |
| Python command | `C:\Env\python\3.14.7\python.exe` before the WindowsApps alias; `Python 3.14.7` |
...
- `where python` -> `C:\Env\python\3.14.7\python.exe`, then the WindowsApps alias
