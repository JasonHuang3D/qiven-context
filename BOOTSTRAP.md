...
10. Resolve the applicable ContextView for the current human/agent combination. View selection may use authenticated human identity plus current agent family/capabilities, but a view never overrides canonical project truth or governance. Registered views include `views/bindings/chatgpt-jason.yaml` (ChatGPT + Jason), `views/bindings/zcode-jason.yaml` (ZCode + Jason), and `views/bindings/extended-cognition-jason.yaml` (ZCode + Jason under the owner-granted `jason-extended-cognition` designation); load the matching view with its referenced human-preference, environment, and workflow profiles. Role-to-model bindings in a view are resolved `verify_live` per ADR-0035; substitution or degradation of the serving model must be disclosed, never silently assumed. Exception (owner direction 2026-09-22): an explicitly owner-invoked designation view may carry no model binding — the owner knows the serving model, and the serving LLM reports the live model family, variant and reasoning at designation switch, with non-introspectable fields reported as such. If no matching view exists, continue with the identity-independent ProjectContext rather than inventing one. If the applicable view cannot be determined with confidence, ask the human operator; never assume or impersonate a participant combination such as chatgpt-jason or zcode-jason.
...
13. Run task-specific retrieval before using project-history facts. Prefer the accepted derived retrieval path when available. Retrieval results are candidate evidence, not truth. (The Python retrieval tooling is sealed museum material per ADR-0041; retrieval is currently performed by the reading agent until the Runtime program provides the capability.)
14. Load task-relevant project material, canonical memory, accepted decisions, rejected alternatives, and non-terminal obligations. Read `memory/index.yaml` and treat every `active` record's title as binding standing knowledge — active memory records are load-bearing (the 2026-09-23 heredoc recurrence: an active rule lived only in memory/records and a cold boot that never swept the index "forgot" it; constitution §16). Retrieve the full record when a title is task-relevant or when unsure.
...
Repository naming, layout and script conventions are canonical in
`qiven-devkit/docs/conventions/` (C++ naming, filesystem layout, script
naming, CMake naming and usage) and apply to every `qiven-*` repository,
including this one. Read the index before creating files, folders, branches
or targets.
