...
from hybrid_retriever import DEFAULT_RRF_K, DEFAULT_TOP_K  # noqa: E402
...
def evaluate_case(
    case: Mapping[str, Any], retriever: StructuralRetriever, *, top_k: int = DEFAULT_TOP_K
) -> dict[str, Any]:
    ...
    for canonical_id in sorted(required | forbidden):
        ...
        diagnostics[canonical_id] = {
            ...
            "structural_rank": hit.structural_rank if hit is not None else None,
            ...
            "graph_rank": hit.graph_rank if hit is not None else None,
            "scope_compatible": hit.scope_compatible if hit is not None else False,
        }
...
def evaluate_benchmark(
    benchmark: Mapping[str, Any], retriever: StructuralRetriever, *, top_k: int = DEFAULT_TOP_K
) -> dict[str, Any]:
    cases = [evaluate_case(case, retriever, top_k=top_k) for case in benchmark["cases"]]
    ...
    return {
        ...
        "top_k": top_k,
        "rrf_k": DEFAULT_RRF_K,
        "graph_seed_top_k": retriever.graph_seed_top_k,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Measure scope-aware, relation-augmented structural hybrid retrieval."
    )
    parser.add_argument(
        "--benchmark",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="FastEmbed model used by the semantic rank channel.",
    )
...
def main() -> int:
    ...
    try:
        ...
        retriever = StructuralRetriever(ROOT, semantic_model=args.semantic_model)
    ...
    if args.json:
        ...
        print("Mode: structural-hybrid")
        print(
            f"Semantic model: {result['semantic_model']} "
            f"top_k={result['top_k']} rrf_k={result['rrf_k']} "
            f"graph_seed_top_k={result['graph_seed_top_k']} graph_depth={result['graph_depth']}"
        )
        for case in result["cases"]:
            ...
            for canonical_id, diagnostic in case["rank_diagnostics"].items():
                ...
                print(
                    ...
                    f"T={diagnostic['structural_rank']} "
                    f"H={diagnostic['hybrid_rank']} "
                    f"D={diagnostic['deterministic_rank']} "
                    f"S={diagnostic['semantic_rank']} "
                    f"G={diagnostic['graph_rank']} "
                    f"scope={'yes' if diagnostic['scope_compatible'] else 'no'}"
                )
