...
class ColdBootContractTests(unittest.TestCase):
    ...
    def test_bootstrap_uses_v2_remote_authority_material_order_and_context_view(self):
        ...
        self.assertIn("views/bindings/chatgpt-jason.yaml",bootstrap)
    ...
    def test_operator_human_contract_is_cross_platform_and_view_bound(self):
        ...
        for required in (
            ...
            "active ContextView",
            "views/bindings/chatgpt-jason.yaml",
        ):
            self.assertIn(required,contract)
    ...
    def test_context_view_separates_project_truth_from_jasonpc_workflow(self):
        ...
        view=yaml.safe_load(self.read("views/bindings/chatgpt-jason.yaml"))
        ...
        # Public-repo information hygiene (2026-09-24): the profile
        # carries placeholder categories resolved live on the machine,
        # never machine-identity literals.
        self.assertEqual(env["workspace"]["qiven_root"],"<workspace-root>")
        self.assertEqual(env["managed_environment"]["root"],"<tool-install-root>")
        self.assertIn("split-routing VPN",env["network"]["posture"])
