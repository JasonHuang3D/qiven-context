...
""",encoding="utf-8"); idx=yaml.safe_load((d/"memory/index.yaml").read_text()); idx["records"].append({"id":name,"file":p.name,"title":"Sample","status":"active"}); (d/"memory/index.yaml").write_text(yaml.safe_dump(idx,sort_keys=False)); return p
    ...
    def test_manual_trigger_without_reason(self): d=self.copy(); p=next((d/"obligations").glob("OBL-*.md")); txt=p.read_text().replace("type: after","type: manual"); txt=re.sub(r"  value: .+\n","",txt, count=1); p.write_text(txt); self.assertIn("reason",self.errors(d))
